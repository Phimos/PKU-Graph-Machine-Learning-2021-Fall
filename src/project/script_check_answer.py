...
def check_answer(pred_path, target_path, metric='rmse'):
    pred = pd.read_csv(pred_path).values[:, 1:].astype(float)
    target = pd.read_csv(target_path).values[:, 1:].astype(float)
    ...
    if metric == 'rmse':
        ...
        pred, target = pred.astype(np.int32), target.astype(np.int32)
        list_roc = []
...
check_answer('answer/sars-pred.csv',
             'data/csvs/sars-eval.csv', metric='roc-auc')
