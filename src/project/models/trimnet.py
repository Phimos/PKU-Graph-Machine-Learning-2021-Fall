...
class MultiHeadTripletAttention(MessagePassing):
    def __init__(self, node_channels, edge_channels, heads=3, negative_slope=0.2, **kwargs):
        super(MultiHeadTripletAttention, self).__init__(
            aggr='add', node_dim=0, **kwargs)  # aggr='mean'
        # node_dim = 0 for multi-head aggr support
        self.node_channels = node_channels
        self.heads = heads
        self.negative_slope = negative_slope
        self.weight_node = Parameter(torch.Tensor(
            node_channels, heads * node_channels))
        self.weight_edge = Parameter(torch.Tensor(
            edge_channels, heads * node_channels))
        self.weight_triplet_att = Parameter(
            torch.Tensor(1, heads, 3 * node_channels))
        self.weight_scale = Parameter(torch.Tensor(
            heads * node_channels, node_channels))
...
class Block(torch.nn.Module):
    def __init__(self, dim, edge_dim, heads=4, time_step=3):
        ...
        self.conv = MultiHeadTripletAttention(
            dim, edge_dim, heads)  # GraphMultiHeadAttention
...
class TrimNet(torch.nn.Module):
    def __init__(self, in_channels, out_channels, edge_dim, hidden_dim=128, depth=2, 
                 heads=4, output_head_channels=512, process_step=2, dropout=0.1):
        ...
        self.set2set = Set2Set(hidden_dim, processing_steps=process_step)
        # self.lin1 = torch.nn.Linear(2 * hidden_dim, 2)
        self.out = nn.Sequential(
            nn.Linear(2 * hidden_dim, output_head_channels),
            nn.LayerNorm(output_head_channels),
            nn.ReLU(inplace=True),
            nn.Dropout(p=self.dropout),
            nn.Linear(output_head_channels, out_channels)
        )

    def forward(self, raw, edge_index, edge_attr, batch):
        x = F.celu(self.lin0(raw))
        for conv in self.convs:
            x = x + F.dropout(conv(x, edge_index, edge_attr),
